...
from transformers import (AdamW, T5Tokenizer, BartTokenizer, BartForConditionalGeneration, T5ForConditionalGeneration, WEIGHTS_NAME,CONFIG_NAME, Adafactor)
...
class DST_Seq2Seq(pl.LightningModule):
    ...
    def configure_optimizers(self):
        return Adafactor(self.parameters(), lr=self.lr, relative_step=False, scale_parameter=False)
...
def train(args, *more):
    ...
    task = DST_Seq2Seq.load_from_checkpoint(args=args, tokenizer=tokenizer, model=model, checkpoint_path=args['load_from_checkpoint'])
    ...
    for batch in tqdm(test_loader):
        ...
        for idx, value in enumerate(value_batch):
            ...
            if dial_id not in predictions:
                predictions[dial_id] = {}
                predictions[dial_id]["domain"] = batch["domains"][idx]
...
def test(args, *more):
    ...
    if "t5" in args["model_name"]:
        model = T5ForConditionalGeneration.from_pretrained(args["model_checkpoint"])
        # model.load_state_dict(torch.load('save/N-Step-Checkpoint_epoch=1_global_step=12900.ckpt'))
        tokenizer = T5Tokenizer.from_pretrained(args["model_checkpoint"], bos_token="[bos]", eos_token="[eos]", sep_token="[sep]")
        model.resize_token_embeddings(new_num_tokens=len(tokenizer))
